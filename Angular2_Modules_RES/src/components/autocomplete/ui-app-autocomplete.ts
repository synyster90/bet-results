...
export class Autocomplete implements AfterViewInit, OnChanges {
    ...
    ngAfterViewInit() {
        setTimeout(() => {
            ...
            $('input', this.element.nativeElement).on('blur', event => {
                if (this.isListOpen)
                    if (!$.contains($('div.ui-autocomplete-list', this.element.nativeElement)[0], $(':focus')[0]) && !$(':focus').hasClass('autocomplete-toggle'))
