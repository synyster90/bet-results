...
var ScommesseService = (function () {
    function ScommesseService(utilService) {
        ...
        this.scommesseList = this.reverseAarrayToMap([{
                value: '1',
                des: '1'
            }, {
                value: 'x',
    ...
    ;
    ScommesseService.prototype.reverseAarrayToMap = function (array) {
        return array.map(function (item) {
            return {
...
var Home = (function () {
    ...
    Home.prototype.getAbilOperations = function () {
        ...
        this.translateService.get('BUTTON_TEXT.DELETE_TITLE').subscribe(function (value) {
            optionDelete.text = value;
        });
        optionDelete.clickEvent.subscribe(function (index) {
                        ...
                        for (var j = 0; j < liveData.matches.length; j++)
                            if (liveData.matches[j].id == _this.scommesseList[i]['match_id']) {
                                if (liveData.matches[j].status == 'fixture') {
                                    ...
                                    var timestamp = parseInt(_this.scommesseService.getMatchInfo(liveData.matches[j].id)['date_time_utc_moment']);
                                    _this.scommesseList[i]['time'] = datePipe.transform(new Date(timestamp * 1000), 'dd/MM/yyyy HH:mm');
                                }
                                else
...
var ViewModalCtrl = (function () {
    ...
    ViewModalCtrl.prototype.deleteItemPopup = function () {
        this.modalDialogService.hide({
            'action': 'delete'
        });
    };
